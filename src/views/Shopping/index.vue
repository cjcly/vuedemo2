<template>
  <div class="cart">
      ...
      <main class="cart-body" v-if="cartGoods.length > 0">
        <div class="wrap">
          <ul class="goods-wrap">
            <li v-for="(item,index) in cartGoods" :key='index' class="cartGoodsItem">
              <cart-one-item
                :itemId="item.id"
                :imgUrl="item.img"
                :title="item.title"
            ...
            <div class="pay">
              ...
              <div>
                <span class="text-left">合计：
                  <!-- 这里面的pay相当于执行了computed对象里面的pay函数，得到了return出来的结果 -->
...
<script>
import CartOneItem from '@/components/CartOneItem.vue'
